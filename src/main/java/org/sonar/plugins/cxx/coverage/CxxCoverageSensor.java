...
public class CxxCoverageSensor extends CxxReportSensor {
	...
	public void analyse(Project project, SensorContext context) {
		...
		List<File> itReports = getReports(conf, project.getFileSystem()
				.getBasedir().getPath(), IT_REPORT_PATH_KEY,
		...
		List<File> overallReports = getReports(conf, project
				.getFileSystem().getBasedir().getPath(),
		...
		saveMeasures(project, context, overallCoverageMeasures,
			...
			for (CoverageParser parser : parsers) {
				try {
					...
					if (!measuresForReport.isEmpty()) {
						...
						CxxUtils.LOG
								.info("Added report '{}' (parsed by: {}) to the coverage data",
										report, parser);
						break;
					}
				} catch (XMLStreamException e) {
					CxxUtils.LOG.trace("Report {} cannot be parsed by {}",
							report, parser);
				}
			}
	...
	private void saveMeasures(Project project, SensorContext context,
		...
		for (Map.Entry<String, CoverageMeasuresBuilder> entry : coverageMeasures
			...
			org.sonar.api.resources.File cxxfile = org.sonar.api.resources.File
					.fromIOFile(javaFile, project);
			if (fileExist(context, cxxfile) && !excluder.isExcluded(javaFile)) {
				CxxUtils.LOG.debug("Saving coverage measures for file '{}'",
						filePath);
	...
	Measure convertToItMeasure(Measure measure) {
		...
		if (CoreMetrics.LINES_TO_COVER.equals(metric)) {
			...
			itMeasure = new Measure(CoreMetrics.IT_COVERAGE_LINE_HITS_DATA,
					measure.getData());
			...
			itMeasure = new Measure(CoreMetrics.IT_COVERED_CONDITIONS_BY_LINE,
					measure.getData());
		} else if (CoreMetrics.CONDITIONS_BY_LINE.equals(metric)) {
			itMeasure = new Measure(CoreMetrics.IT_CONDITIONS_BY_LINE,
					measure.getData());
		}
	...
	private Measure convertForOverall(Measure measure) {
		...
		if (CoreMetrics.LINES_TO_COVER.equals(measure.getMetric())) {
			itMeasure = new Measure(CoreMetrics.OVERALL_LINES_TO_COVER,
					measure.getValue());
		} else if (CoreMetrics.UNCOVERED_LINES.equals(measure.getMetric())) {
			itMeasure = new Measure(CoreMetrics.OVERALL_UNCOVERED_LINES,
					measure.getValue());
		} else if (CoreMetrics.COVERAGE_LINE_HITS_DATA.equals(measure
				.getMetric())) {
			itMeasure = new Measure(
					CoreMetrics.OVERALL_COVERAGE_LINE_HITS_DATA,
					measure.getData());
		} else if (CoreMetrics.CONDITIONS_TO_COVER.equals(measure.getMetric())) {
			itMeasure = new Measure(CoreMetrics.OVERALL_CONDITIONS_TO_COVER,
					measure.getValue());
		} else if (CoreMetrics.UNCOVERED_CONDITIONS.equals(measure.getMetric())) {
			itMeasure = new Measure(CoreMetrics.OVERALL_UNCOVERED_CONDITIONS,
					measure.getValue());
		} else if (CoreMetrics.COVERED_CONDITIONS_BY_LINE.equals(measure
				.getMetric())) {
			itMeasure = new Measure(
					CoreMetrics.OVERALL_COVERED_CONDITIONS_BY_LINE,
					measure.getData());
		} else if (CoreMetrics.CONDITIONS_BY_LINE.equals(measure.getMetric())) {
			itMeasure = new Measure(CoreMetrics.OVERALL_CONDITIONS_BY_LINE,
					measure.getData());
		}
	...
	private boolean fileExist(SensorContext context,
